...
class Final (object):
  ...
  def do_final (self, packet, packet_in, port_on_switch, switch_id):
    ...
    if ip is not None:
      ...
      # Handles ICMP or TCP packet
      if ip_packet.protocol == ip_packet.ICMP_PROTOCOL or ip_packet.protocol == ip_packet.TCP_PROTOCOL:
        # drop packet if d1 or d2 is the src or dest
        if ip_packet.srcip in homeAddresses or ip_packet.dstip in homeAddresses:
          drop()
        else:
